...
# Script to build and run the Polymarket v103 MCP Server locally in Docker
...
IMAGE_NAME="polymarket-v103-mcp-server"
CONTAINER_NAME="polymarket-v103-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Polymarket v103 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Polymarket v103 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Polymarket v103 MCP Server is running!${NC}"
echo
